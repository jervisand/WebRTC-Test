...
public class Reporter : MonoBehaviour
{
	...
	float gcTotalMemory;
	public Nagih.CircleGestureDetector CircleDetector;
	...
	Vector2 getDownPos()
	{
		   ...
		   Application.platform == RuntimePlatform.IPhonePlayer)
		{
			...
			var touchCount = touches.Length;
#endif
			if (touchCount == 1 && touches[0].phase == TouchPhase.Began)
			{
#if ENABLE_INPUT_SYSTEM
				downPos = touches[0].screenPosition;
#else
...
#if ENABLE_INPUT_SYSTEM
			return touches[0].screenPosition - downPos;
#else
			return touches[0].position - downPos;
#endif
		}
		else
		{
	...
	void Update()
	{
		...
		calculateStartIndex();
		if (!show && CircleDetector.IsGestureDone())
		{
			doShow();
		}
